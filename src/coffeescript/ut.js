...
var B_FAIL_FAST, Base, INFINITE_LOOP_DETECTION_ITERATIONS, O, S, T, TESTNBR, UT, UT_UT, V, aGenerate, bHappy, bRan, bRunning, bag, decorate, decorateJustObject, fail, g_timer, handler, iterations, msStart, pass, path, proxyBag, sectionGenerate, t_depth, target, testGenerate, testIndex, testList, testListSaved, testStack, trace, util;
...
decorateJustObject = function(test, me2, parent) {
  ...
  me2.eq = function() {
    var _, bEQ, bStrings, i, j, l, m, n, p, q, ref, ref1, ref2, ref3, ref4, ref5, s;
    bEQ = true;
...
//			process.exit 1
      ...
      if (bEQ) {
        ...
        if (!bEQ) {
          ...
          if (bStrings) {
            s += "---------------------LEN------------------\n";
            for (i = p = 0, ref4 = arguments.length - 1; (0 <= ref4 ? p <= ref4 : p >= ref4); i = 0 <= ref4 ? ++p : --p) {
              s += `length=${arguments[i].length}\n`;
            }
            s += "---------------------DUMP------------------\n";
            for (i = q = 0, ref5 = arguments.length - 1; (0 <= ref5 ? q <= ref5 : q >= ref5); i = 0 <= ref5 ? ++q : --q) {
              s += `${S.HEX(arguments[i])}\n\n`;
            }
          }
          s += "-------------------------------------------\n";
...
module.exports = UT = (function() {
  class UT extends Base {
    ...
    argsProcess(a) {
      ...
      while (i < a.length) {
        ...
        switch (item) {
          ...
          case "-grep":
            pattern = a[i++];
            console.log(S.autoTable(testList, {
              bHeader: true,
              grep: pattern,
            ...
            break;
          case "-h":
            console.log(`node tests.js [options]\n\nOPTIONS:${S.autoTable(optionList, {
              bHeader: false
            })}`);
            console.log;
            process.exit(1);
    ...
    next() {
      var bExpectException, ex, failSave, fnBoundObjectThis, handle, passSave, prNOT_USED, ref, ref1, rv, test, testThis;
      if (!bRunning) {
        return;
      }
    ...
    run() { //H: UT should know NOTHING about "TestHub"
      var j, k, len, mTypeCtrList, opts, ref, ref1, summary, test;
      this["@who"] = "UNIT TEST RUNNER";
      this.argsProcess(process.argv.slice(2));
      if (typeof this.fnCallback === "function") {
...
//				@log "opts", opts
          for (k in opts) {
            if (k !== "desc" && k !== "expect" && k !== "hang" && k !== "human" && k !== "internet" && k !== "mType" && k !== "onError" && k !== "onException" && k !== "onTimeout" && k !== "SO" && k !== "RUNTIME_SECS" && k !== "timeout" && k !== "url" && k !== "USER_CNT") {
                ...
                if (bHappy) {
                  ...
                  if (fail) {
                    ...
                    if (trace.TRACE_DURATION_REPORT && testList.length) {
                      ...
                      for (i = l = 0, ref2 = testList.length - 1; (0 <= ref2 ? l <= ref2 : l >= ref2); i = 0 <= ref2 ? ++l : --l) {
                        test = testList[i];
