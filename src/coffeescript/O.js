...
(function() {
  // process: ENV={"node":true,"rn":false,"ut":true,"source":"/Users/pete/gitlab/rn/API/Flexbase/O.coffee"}
  var A_CLR_ENUM, CLR_ENUM, CNT, CNT_ENUM, CNT_ENUM_OWN, CNT_OWN, CONTAINS_INSENSITIVE, DEEP, DEEP_DNW, DEEP_OPPOSITE, DEEP_TOO_MUCH_WORK, DFS_BREAKABLE, DUMP, EXPORTED, IS_EMPTY, KEYS, KEYS_ENUM, KEYS_ENUM_OWN, KEYS_OWN, LOG, LOG_DRILL, O, SUMMARY, V, duck, g_LOGIgnore, trace;
        ...
        switch (dt = IS.dt(v)) {
          case "a":
            ...
            for (idx = l = 0, len = v.length; l < len; idx = ++l) {
              v2 = v[idx];
              dt = IS.dt(v);
              if (!(bContinue = fn(v, idx, v2, dt, depth))) {
                return false;
              }
              if (dt === "o") {
                if (!(bContinue = DFS_(v2, depth + 1))) {
                  return false;
                }
              }
            }
